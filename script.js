...
function initCloudSync() {
    db.enableNetwork().then(() => console.log("Firebase OK")).catch(() => console.log("Offline mode"));
    budgetDocRef.onSnapshot((doc) => {
        if (doc.exists) {
            LOCAL_DATA = doc.data();
            normalizeData();
            if (checkAuth()) renderAll();
        } else { budgetDocRef.set(LOCAL_DATA); }
    });
}
function saveToCloud() {
    budgetDocRef.set(LOCAL_DATA).catch(() => showToast('Erro ao salvar online', 'error'));
...
function isItemActiveForMonth(item, targetMonth, targetYear) {
    return item.activeMonths && item.activeMonths.includes(getMonthKey(targetMonth, targetYear));
}
...
function renderCalendar() {
    ...
    for(let i=0; i<12; i++) {
        ...
        const btn=document.createElement('div'); btn.className=`cal-item ${active?'active':''} ${has?'has-':''}`;
    ...
    document.getElementById('btnCurrentMonth').onclick = () => {
        APP_STATE.currentMonth = new Date().getMonth(); APP_STATE.currentYear = new Date().getFullYear();
        renderAll(); showToast('Voltou para o mês atual! 📅', 'success');
    };
}
...
function renderItems(type) {
    ...
    items.forEach(item=>{
        ...
        let receiptHTML = item.receiptImage ? `<img src="${item.receiptImage}" class="item-receipt-thumb" onclick="viewFullImage('${item.receiptImage}')" title="Ver comprovante">` : '';
...
function setupImageUpload() {
    const uploadArea = document.getElementById('uploadArea'), receiptInput = document.getElementById('receiptInput');
    const placeholder = document.getElementById('uploadPlaceholder'), preview = document.getElementById('uploadPreview');
    const img = document.getElementById('previewImage'), btnRemove = document.getElementById('btnRemoveReceipt');
    uploadArea.addEventListener('click', (e) => { if (e.target !== btnRemove) receiptInput.click(); });
    ...
    receiptInput.addEventListener('change', e => { if(e.target.files[0]) handleFile(e.target.files[0]); });
    btnRemove.addEventListener('click', e => { e.stopPropagation(); currentReceiptImage = null; placeholder.style.display = 'flex'; preview.style.display = 'none'; receiptInput.value = ''; });
    function handleFile(file) {
        if(!file.type.startsWith('image/')) return showToast('Apenas imagens JPG/PNG.', 'error');
        if(file.size > 2*1024*1024) return showToast('Arquivo maior que 2MB.', 'error');
        const reader = new FileReader(); reader.onload = ev => { currentReceiptImage = ev.target.result; img.src = currentReceiptImage; placeholder.style.display = 'none'; preview.style.display = 'block'; }; reader.readAsDataURL(file);
    }
}
function viewFullImage(src) { document.getElementById('fullImage').src = src; document.getElementById('imageModalOverlay').classList.add('active'); }
document.getElementById('btnCloseImageModal').onclick = () => document.getElementById('imageModalOverlay').classList.remove('active');
document.getElementById('imageModalOverlay').onclick = (e) => { if(e.target === e.currentTarget) document.getElementById('imageModalOverlay').classList.remove('active'); };

// MODAL
function renderMonthGrid(selected = []) {
    const grid = document.getElementById('monthsCheckGrid'); grid.innerHTML = '';
    for (let i = 0; i < 12; i++) {
        const m = (APP_STATE.currentMonth + i) % 12; const y = APP_STATE.currentYear + Math.floor((APP_STATE.currentMonth + i) / 12);
        const key = getMonthKey(m, y); const checked = selected.includes(key);
...
function openModal(type, id=null) {
    const ov=document.getElementById('modalOverlay'), title=document.getElementById('modalTitle'), catGrp=document.getElementById('categoryGroup'), occGrp=document.getElementById('occurrenceGroup'), recGrp=document.getElementById('receiptGroup');
    document.getElementById('itemType').value=type; document.getElementById('itemId').value=id||'';
    if(type==='expense'){catGrp.style.display='block';occGrp.style.display='block';recGrp.style.display='block';title.textContent=id?'Editar Despesa':'Adicionar Despesa';}
    else{catGrp.style.display='none';occGrp.style.display='none';recGrp.style.display='none';title.textContent=id?'Editar Receita':'Adicionar Receita';}
    document.getElementById('smartSuggestion').classList.remove('show');
    currentReceiptImage = null; document.getElementById('uploadPlaceholder').style.display = 'flex'; document.getElementById('uploadPreview').style.display = 'none'; document.getElementById('receiptInput').value = '';
    if(id){
        ...
        if(item){
            ...
            if(type==='expense'){
                ...
                if(item.receiptImage) { currentReceiptImage=item.receiptImage; document.getElementById('previewImage').src=item.receiptImage; document.getElementById('uploadPlaceholder').style.display='none'; document.getElementById('uploadPreview').style.display='block'; }
            }
        }
    }else{
        document.getElementById('itemForm').reset();
        if(type==='expense'){ document.querySelector('input[name="occurrenceType"][value="single"]').checked = true; document.getElementById('monthSelectionGrid').style.display = 'none'; }
    }
    document.querySelectorAll('input[name="occurrenceType"]').forEach(r => r.onchange = e => { document.getElementById('monthSelectionGrid').style.display = e.target.value==='recurring'?'block':'none'; if(e.target.value==='recurring') renderMonthGrid(id?(LOCAL_DATA.recurringItems.find(i=>i.id===id)?.activeMonths||[]):[]); });
...
function saveItem(e){
    ...
    let activeMonths = [];
    if(type==='expense' && document.querySelector('input[name="occurrenceType"]:checked').value === 'recurring') document.querySelectorAll('#monthsCheckGrid input:checked').forEach(cb => activeMonths.push(cb.value));
    ...
    if(type==='expense' && currentReceiptImage) newItem.receiptImage = currentReceiptImage;
    if(editId){
        if(activeMonths.length>1){const idx=LOCAL_DATA.recurringItems.findIndex(i=>i.id===editId);if(idx!==-1){if(!newItem.receiptImage&&LOCAL_DATA.recurringItems[idx].receiptImage) newItem.receiptImage=LOCAL_DATA.recurringItems[idx].receiptImage; LOCAL_DATA.recurringItems[idx]=newItem;}showToast('Atualizado para os meses selecionados! 🔄');}
...
function downloadExcelTemplate() {
    const wb = XLSX.utils.book_new(); const data = [{ Tipo: 'Receita ou Despesa', Descrição: 'Ex: Aluguel', Categoria: 'moradia/alimentacao/etc', Valor: 1000, Meses_Ativos: '2024-05,2024-06,2024-07', Dia_Vencimento: 10 }];
    const ws = XLSX.utils.json_to_sheet(data); ws['!cols'] = [{wch:20},{wch:30},{wch:20},{wch:10},{wch:35},{wch:10}];
    XLSX.utils.book_append_sheet(wb, ws, "Modelo"); XLSX.writeFile(wb, "Modelo_Importacao_Excel.xlsx"); showToast('Modelo baixado! 📥');
}
function exportExcel() { const rows=[]; LOCAL_DATA.recurringItems.forEach(i=>rows.push({'Tipo':i.type==='income'?'Receita':'Despesa','Descrição':i.description,'Categoria':i.category||'-','Valor':i.amount,'Meses_Ativos':(i.activeMonths||[]).join(','),'Dia_Vencimento':i.dueDate||'-'})); const ws=XLSX.utils.json_to_sheet(rows), wb=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,"Orçamento"); XLSX.writeFile(wb,`Orcamento_${new Date().toISOString().slice(0,10)}.xlsx`); showToast('Planilha exportada! 📊'); }
...
initCloudSync();
if(checkAuth()){
    const checkData = setInterval(() => { if (LOCAL_DATA.recurringItems !== undefined) { clearInterval(checkData); renderAll(); } }, 500);
}else{
