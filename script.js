/* ============================================
   ORÇAMENTO FAMILIAR - V6.0 (FIREBASE SYNC)
   ============================================ */

// ⚠️ PASSO CRÍTICO: COLE SUAS CHAVES DO FIREBASE AQUI
// Vá em Console Firebase > Configurações do Projeto > Geral > Seus Apps > SDK setup e config
...
function initCloudSync() {
    ...
    budgetDocRef.onSnapshot((doc) => {
        if (doc.exists) {
            LOCAL_DATA = doc.data();
            // Normalizar dados ao receber da nuvem
            normalizeData();
            if (checkAuth()) renderAll();
            showToast('Sincronizado ☁️', 'success');
        } else {
            // Se não existe na nuvem, cria com dados locais ou vazios
            budgetDocRef.set(LOCAL_DATA);
        }
    }, (error) => {
        console.error("Erro na sincronização:", error);
        showToast('Erro ao conectar com nuvem. Verifique o config.', 'error');
    });
}
...
function saveItem(e){
    ...
    if(editId){
        if(activeMonths.length>1){const idx=LOCAL_DATA.recurringItems.findIndex(i=>i.id===editId);if(idx!==-1)LOCAL_DATA.recurringItems[idx]=newItem;showToast('Atualizado para os meses selecionados! 🔄');}
...
// Boot do App
initCloudSync(); // Começa a sincronização
if(checkAuth()){
    // Se já logado, espera o dado chegar da nuvem
